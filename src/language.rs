//! Language dot colors, the ones GitHub's own language bar uses.
//!
//! Vendored from Linguist's languages.yml (github-linguist/linguist, MIT),
//! which is where GitHub reads them too, so the dot on a row matches the dot
//! on the repo's own page. Names are sorted for the binary search below.
//!
//! Generated file, do not edit: regenerate from languages.yml when the
//! palette drifts.

/// The hex behind a language's dot, or None for a language Linguist gives
/// no color, which the caller turns into the neutral dot.
pub fn color(language: &str) -> Option<&'static str> {
    COLORS
        .binary_search_by_key(&language, |&(name, _)| name)
        .ok()
        .map(|i| COLORS[i].1)
}
...
mod tests {
    ...
    #[test]
    fn the_table_is_sorted_or_the_search_lies() {
        assert!(COLORS.windows(2).all(|w| w[0].0 < w[1].0));
    }

    #[test]
    fn knows_the_dots_github_shows() {
        assert_eq!(color("Rust"), Some("#dea584"));
        assert_eq!(color("Markdown"), Some("#083fa1"));
        assert_eq!(color("Shell"), Some("#89e051"));
        assert_eq!(color("Fable"), None);
    }
}
